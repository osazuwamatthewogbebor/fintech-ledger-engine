...
as a column that gets updated. Instead they're derived by summing an
append-only list of ledger entries, so the history is the source of truth
and nothing can change a balance without leaving a record.
...
- **No reversal or correction flow.** Since entries are immutable, fixing a
  mistake should mean writing a compensating transaction, which isn't
  implemented.
